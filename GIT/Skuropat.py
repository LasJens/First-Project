...
canvas = Canvas(tk, width=1000, height=800, bg="#99FFFF")

lava1 = canvas.create_rectangle(0, 800, 600, 785, fill='red')
lava2 = canvas.create_rectangle(700, 800, 1000, 785, fill='red')

wall_left = canvas.create_rectangle(0, 0, 15, 800, fill='blue')
wall_right = canvas.create_rectangle(985, 0, 1000, 800, fill='blue')
wall_ceiling = canvas.create_rectangle(0, 0, 1000, 15, fill='blue')

button_gates = canvas.create_rectangle(15, 300, 30, 340, fill="#006633")
...
class Ball:
    ...
    def __init__(self, canvas, paddle, color):
        ...
        start_x = [150, 200, 250, 300, 600, 650, 700, 750]
        start_y = [50, 100, 150, 200]
        random.shuffle(start_x)
        random.shuffle(start_y)
        self.starting_point_x = start_x[0]
        self.starting_point_y = start_y[0]
    ...
    def draw(self):
        ...
        if ((pos[3] >= self.canvas_height) and ((pos[0] >= 600) and (pos[2] <= 700))):
            ...
            self.hit_bottom = True
            # выводим сообщение
            canvas.create_text(500, 400, text='Скуропат спасен!', font=('Courier', 30), fill='#006633')

        #cоздаем угловые спасительные ворота
        if ((pos[0] <= 30) and (pos[1] >= 270) and (pos[3] <= 360)):
            saving_gates = canvas.create_polygon(850, 15, 985, 15, 985, 150, fill="#006633")
            global flag_gates
            flag_gates = True
       ...
       #кнопка создания отражающего квадрата
        if ((pos[0] <= 30) and (pos[1] >= 40) and (pos[3] <= 130)):
            reflect_square = canvas.create_rectangle(460, 170, 540, 250, fill="#FF00FF")
            global flag_square
            flag_square = True
...
class Paddle:
    ...
    def __init__(self, canvas, color):
        ...
        start_x = [200, 300, 400, 500, 600, 700, 800]
        # перемешиваем их
        random.shuffle(start_x)
...
paddle = Paddle(canvas, "#660066")
# создаём объект — бежевую рыбку-шарик
ball = Ball(canvas, paddle, "#FFCC33")
